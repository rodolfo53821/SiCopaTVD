...
class DatagramSocket {
    ...
    static Method isBcastMeth = null;
	...
	try {
	    Class interfaceMapCl = Class.forName("com.sun.tv.net.InterfaceMapImpl");
	    Class argCl[] = new Class[1];
	    argCl[0] = (new InetAddress()).getClass();

	    isBcastMeth = interfaceMapCl.getMethod("isBcastEncapIP",argCl);
	...
	// For broadcast data -- 
	// find if the laddr is one of the javax.tv.net assigned address for
	// receiving encapsulated IP packets. 
	// if so, change implClass to EncapDatagramSocketImpl
	// Vector addrVec = com.sun.tv.net.InerfaceMap.getIPAddrs();
	// for (e = addrVec.element(); addrVec.hasMoreElements();) {
	//    if ( laddr.equal((InetAddress)e)) implClass = ...
	
	try {
	    Object argMeth[] = new Object[1];
	    argMeth[0] = laddr;
	    if (isBcastMeth != null) {
		Boolean bcastIP = (Boolean) isBcastMeth.invoke(null, argMeth);
		if (bcastIP.booleanValue()) {
		    try {
			System.out.println("dsock: making Encap implclass");
			instanceImplClass = Class.forName("com.sun.tv.net.EncapDatagramSocketImpl");
		    } catch (Exception e) {
			e.printStackTrace();
			// EncapDatagramSocketImpl doesn't exist, keep the old impl class
		    }
		}
		else {
		    instanceImplClass = implClass;
		}
	    } else {
		instanceImplClass = implClass;
	    }
	    ...
	    e.printStackTrace();
	    // one of the class/methods doesn't exist, keep the old impl class
	    instanceImplClass = implClass;
	}
	...
	try {
	    System.out.println("my dsocket -- implClass ="+instanceImplClass.getName());
	    impl = (DatagramSocketImpl) instanceImplClass.newInstance();
	} catch (Exception e) {
