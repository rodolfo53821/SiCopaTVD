...
class MulticastSocket extends DatagramSocket {
     ...
     * @since   JDK1.1
     */
    public synchronized void setInterface(InetAddress inf) throws SocketException {
	...
	try {
		...
		if (bcastIP.booleanValue()) {
		    try {
			System.out.println("Multisock: making new implclass");
			instanceImplClass = Class.forName("com.sun.tv.net.EncapDatagramSocketImpl");
		    } catch (Exception e) {
			throw new SocketException("can't form class from com.sun.tv.net.EnvapDatagramSocketImpl" + e.toString());
		    }
		}
	    }
	} catch (Exception e) {
	...
	if ( impl != null ) { // if an impl is already created at 
	    // constructor level, close it.
	    impl.close();
	}
	
	try {
	    this.impl = (DatagramSocketImpl) instanceImplClass.newInstance();
	} catch (Exception e) {
	    throw new SocketException("can't instantiate DatagramSocketImpl" + e.toString());
	}
	impl.create();
	impl.setOption(SocketOptions.SO_REUSEADDR, new Integer(-1));
	impl.bind(this.port, inf);
